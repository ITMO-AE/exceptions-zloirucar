...
Rational const operator /(const Rational &vector1, const Rational &vector2)
{
    vector2.Denominator() == 0 || vector2.Numerator() == 0 ? throw std::domain_error("Делитель не должен равняться нулю"): false;
...
bool operator ==(const Rational &vector1, const Rational &vector2)
{
...
void Rational::nod()
{
    ...
    if (l_denominator == 0)
    {
        throw std::invalid_argument("Делитель равен нулю ");
    }
